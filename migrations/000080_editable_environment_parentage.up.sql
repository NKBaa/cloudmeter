...
-- 000079 normalized editable environment values in the wrong direction: it
-- copied user values into the template and therefore returned a runtime that
-- could never equal the immutable product version. Strip the declared
-- editable keys from both sides, require every fixed key to remain identical,
-- then restore the administrator template before the parentage comparison.
...
BEGIN
    IF normalized IS NULL OR expected IS NULL OR jsonb_typeof(options) IS DISTINCT FROM 'object'
       ...
       OR jsonb_typeof(template_env) IS DISTINCT FROM 'object'
    ...
    FOR env_key IN SELECT jsonb_array_elements_text(editable_env) LOOP
        IF NOT (template_env ? env_key) OR NOT (release_env ? env_key)
           OR jsonb_typeof(release_env->env_key) <> 'string' THEN RETURN NULL; END IF;
        release_env := release_env - env_key;
        template_env := template_env - env_key;
    END LOOP;
    IF release_env IS DISTINCT FROM template_env THEN RETURN NULL; END IF;
    IF expected ? 'env' THEN normalized := jsonb_set(normalized,'{env}',expected->'env',true);
    ELSE normalized := normalized - 'env'; END IF;
