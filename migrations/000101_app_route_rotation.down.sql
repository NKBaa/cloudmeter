...
BEGIN
    SELECT coalesce(settings.app_base_domain, '') INTO base_domain FROM system_settings settings WHERE settings.singleton;
    SELECT CASE WHEN base_domain = '' THEN '/apps/'||users.slug||'/'||app.slug
                ELSE '//'||app.slug||'-'||users.slug||'.'||base_domain||'/' END,
...
UPDATE app_routes route SET public_path=CASE
    WHEN settings.app_base_domain='' THEN '/apps/'||users.slug||'/'||app.slug
    ELSE '//'||app.slug||'-'||users.slug||'.'||settings.app_base_domain||'/' END
FROM user_apps app JOIN users ON users.id=app.user_id CROSS JOIN system_settings settings
WHERE route.user_app_id=app.id AND settings.singleton;

DROP INDEX IF EXISTS user_apps_domain_refresh_due_idx;
DROP INDEX IF EXISTS user_apps_active_route_host_label_uidx;
ALTER TABLE user_apps
    DROP CONSTRAINT IF EXISTS user_apps_domain_next_refresh_check,
    DROP CONSTRAINT IF EXISTS user_apps_domain_refresh_days_check,
    DROP CONSTRAINT IF EXISTS user_apps_route_host_label_check,
    DROP COLUMN IF EXISTS domain_next_refresh_at,
    DROP COLUMN IF EXISTS domain_refreshed_at,
    DROP COLUMN IF EXISTS domain_refresh_days,
    DROP COLUMN IF EXISTS route_host_label;
