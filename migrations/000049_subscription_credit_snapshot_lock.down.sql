...
DROP FUNCTION IF EXISTS grant_subscription_credit(uuid,uuid);
...
DECLARE
    ...
    v_ref text;
BEGIN
    IF p_target_cents IS NULL OR p_target_cents <= 0 THEN RETURN; END IF;
    ...
    SELECT coalesce(sum(g.amount_cents),0) INTO v_granted
    ...
    INSERT INTO credit_grants AS grant_row(
        ...
        (date_trunc('month',now() AT TIME ZONE 'UTC') + interval '1 month') AT TIME ZONE 'UTC',p_created_by
    ) ON CONFLICT ON CONSTRAINT credit_grants_user_id_business_ref_key DO NOTHING
    RETURNING grant_row.id INTO v_id;
    IF v_id IS NULL THEN RETURN; END IF;
    id := v_id; amount_cents := v_delta; business_ref := v_ref;
    RETURN NEXT;
END $$;
