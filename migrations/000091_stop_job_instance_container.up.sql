...
-- Container names embed the independent instance identity since migration
-- 000081, but the stop-job guard still validated against the legacy user_app
-- id. Accept both the instance-based and legacy application-based formats,
-- matching the route integrity guard exactly.
...
BEGIN
    ...
    END IF;
    IF NEW.container_name <> '' THEN
        SELECT instance_id INTO app_instance_id FROM user_apps WHERE id = NEW.user_app_id;
        IF app_instance_id IS NULL OR NEW.release_id IS NULL OR NOT (
            NEW.container_name = 'cm-' || app_instance_id::text || '-' || NEW.release_id::text
            OR NEW.container_name ~ ('^cm-[0-9a-f]{10}-' || app_instance_id::text || '-' || NEW.release_id::text || '$')
            OR NEW.container_name = 'cm-' || NEW.user_app_id::text || '-' || NEW.release_id::text
