...
cleanup(){ curl -sS -X PUT -H "Authorization: Bearer $ADMIN_TOKEN" -H 'Content-Type: application/json' -d '{"enabled":false,"merchantId":"","endpoint":"","secret":""}' "$BASE_URL/admin/settings/payments/epay" >/dev/null 2>&1 || true; db "UPDATE sessions SET revoked_at=now() WHERE id IN ('$ADMIN_SESSION','$USER_SESSION')" >/dev/null 2>&1 || true; }
trap cleanup EXIT
...
SIGN="$(printf '%s' "$canonical" | openssl dgst -sha256 -hmac "$SECRET" -binary | xxd -p -c 256)"
...
[[ "$VALID" == *'"success":true'* ]] || { echo 'valid EPay callback rejected' >&2; exit 1; }
REPLAY="$(curl -fsS -X POST "${FORM[@]}" --data-urlencode "sign=$SIGN" "$BASE_URL/payments/epay/callback")"
[[ "$REPLAY" == *'"idempotent":true'* ]] || { echo 'EPay callback replay was not idempotent' >&2; exit 1; }
