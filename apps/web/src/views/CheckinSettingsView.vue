...
async function load() {
  loading.value = true;
  try {
    Object.assign(model, await api("/admin/settings/checkin"));
  } catch (e) {
...
async function save() {
  ...
  message.value = "";
  try {
    await api("/admin/settings/checkin", {
      method: "PUT",
      body: JSON.stringify({
        enabled: model.enabled,
        minRewardCents: model.minRewardCents,
        maxRewardCents: model.maxRewardCents,
      }),
    });
  ...
  <main class="workspace admin-workspace checkin-settings-page">
    <header>
      <div>
        ...
        <h1>签到设置</h1>
        <p class="quiet">
          配置每日签到功能的开关与奖励范围，用户按北京时间签到一次，奖励实时写入不可变赠送账本。
        </p>
      </div>
    </header>
    ...
    <section class="checkin-hero">
      <div class="checkin-hero-inner">
        ...
        <div>
          ...
          <h2>用持续的签到习惯激励用户</h2>
          <p>
            每天北京时间零点重置，用户签到后随机获得区间内奖励，直接进入不可篡改的赠送账本。
          </p>
        </div>
      </div>
      <div class="checkin-hero-meta" v-if="!loading">
        <div class="checkin-meta-chip">
          <Clock :size="15" />
          <div><strong>重置周期</strong><span>北京时间 00:00</span></div>
        </div>
        <div class="checkin-meta-chip">
          <Gift :size="15" />
          <div>
            <strong>奖励区间</strong><span>{{ rewardRange }}</span>
          </div>
        </div>
      </div>
    </section>
      ...
      <form class="checkin-form" @submit.prevent="save">
        <div class="checkin-toggle-row">
          <div class="checkin-toggle-row-text">
            ...
            <small
              >关闭后签到入口对用户隐藏，历史奖励记录不受影响，用户仍可查看已获得的奖励。</small
            >
          </div>
          <label class="switch"
        ...
        <div class="checkin-fields">
          <label class="checkin-field">
            ...
            <div class="checkin-cents-input">
              ...
              <input
                v-model.number="model.minRewardCents"
                type="number"
                min="1"
                :max="model.maxRewardCents"
                step="1"
                required
              />
            </div>
          ...
          <label class="checkin-field">
            ...
            <div class="checkin-cents-input">
              <span class="checkin-cents-currency">¥</span>
              <input
                v-model.number="model.maxRewardCents"
                type="number"
                :min="model.minRewardCents"
                max="10000"
                step="1"
                required
              />
...
.checkin-settings-page > header {
  margin-bottom: 0;
}
.checkin-settings-page > header h1 {
  margin: 2px 0 8px;
  font-size: 22px;
}
.checkin-settings-page > header .quiet {
  margin: 0;
...
.checkin-hero-inner h2 {
  margin: 2px 0 6px;
  font-size: 18px;
}
.checkin-hero-inner p {
  margin: 0;
  color: var(--text-muted);
  font-size: 13px;
  line-height: 1.55;
...
.checkin-meta-chip strong {
  display: block;
  font-size: 13px;
}
.checkin-meta-chip span {
  color: var(--text-muted);
  font-size: 12px;
}
...
.checkin-toggle-row-text strong {
  display: block;
  font-size: 14px;
  margin-bottom: 3px;
}
.checkin-toggle-row-text small {
  color: var(--text-muted);
  font-size: 12px;
  line-height: 1.5;
}
...
.checkin-cents-currency {
  font-size: 18px;
  font-weight: 700;
  color: var(--text-muted);
}
.checkin-cents-input input {
  flex: 1;
  min-width: 0;
  border: 0;
...
.checkin-strip-item strong {
  display: block;
  font-size: 13px;
  margin-bottom: 2px;
}
.checkin-strip-item span {
  color: var(--text-muted);
  font-size: 12px;
}
