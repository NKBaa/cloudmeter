...
async function load() {
  loading.value = true;
  try {
    Object.assign(model.value, await api("/admin/settings/logs"));
  } catch (e) {
...
async function clearAll() {
  if (!confirm("确认清空全部已缓存的实例运行日志？此操作立即生效且不可恢复。"))
    return;
  try {
    clearing.value = true;
...
async function clearAudit() {
  if (!confirm("危险操作：确认清空全部平台审计日志？该表为只读历史，清空后无法追踪过去的敏感操作。此操作不可恢复。"))
    return;
  try {
    clearingAudit.value = true;
    await api("/admin/audit-logs/clear", { method: "POST", body: "{}" });
    message.value = "已清空全部审计日志";
    error.value = "";
  } catch (e) {
    error.value = (e as Error).message;
  } finally {
  ...
  <main class="workspace admin-workspace log-settings-page">
    <header>
      <div>
        ...
        <h1>日志设置</h1>
        <p class="quiet">
          统一管理应用运行日志在平台侧的缓存保留策略，控制磁盘占用与分析窗口。
        </p>
      </div>
    </header>
    ...
    <section class="log-hero">
      <div class="log-hero-inner">
        ...
        <div>
          ...
          <h2>保留多久、多大，由你掌控</h2>
          <p>
            日志按实例缓存，超过保留时长或单条上限的旧内容自动裁剪，避免长期累积占用磁盘。
          </p>
        </div>
      </div>
      <div class="log-hero-meta">
        <div class="log-meta-chip">
          <Clock :size="15" />
          <div>
            <strong>缓存清理</strong><span>超时或超限后按行自动裁剪</span>
          </div>
        </div>
      </div>
    </section>
      ...
      <form class="log-form" @submit.prevent="save">
        <label class="log-field">
          ...
          <div class="log-input-with-unit">
            <input
              v-model.number="model.retentionHours"
              type="number"
              min="1"
              max="8760"
              step="1"
    ...
    <div class="log-summary-strip" v-if="!loading">
      <div class="log-summary-item">
        <span>保留时长</span><strong>{{ retentionSummary }}</strong>
      </div>
      <div class="log-summary-item">
        <span>单条上限</span
        ><strong>{{ humanize(model.retentionBytes) }}</strong>
      </div>
      <div class="log-summary-item">
        <span>空间策略</span><strong>{{ spaceTier }}</strong>
      </div>
    </div>

    <section class="log-danger-panel" style="display: grid; gap: 24px;">
      <div class="log-danger">
        <span class="log-danger-icon"><AlertTriangle :size="20" /></span>
        <div>
          <strong>清空应用运行日志</strong>
          <p>移除所有实例的运行日志缓存，释放磁盘占用。此操作不可恢复。</p>
        </div>
        <button
          type="button"
          ...
          @click="clearAll"
        >
          <Trash2 :size="15" />{{ clearing ? "清理中…" : "清空运行日志" }}
        </button>
      </div>
      ...
      <div class="log-danger">
        <span class="log-danger-icon"><AlertTriangle :size="20" /></span>
        <div>
          <strong>清空平台审计日志</strong>
          <p>移除系统产生的所有的审计操作追踪历史，合规数据将丢失。此操作不可恢复。</p>
        </div>
        <button
          type="button"
          class="danger compact"
          ...
          @click="clearAudit"
        >
          <Trash2 :size="15" />{{ clearingAudit ? "清理中…" : "清空审计日志" }}
        </button>
      </div>
    </section>
  </main>
...
.log-settings-page > header {
  margin-bottom: 0;
}
.log-settings-page > header h1 {
  margin: 2px 0 8px;
  font-size: 22px;
}
.log-settings-page > header .quiet {
  margin: 0;
...
.log-danger-icon {
  ...
  place-items: center;
  border-radius: 12px;
  background: rgba(185, 47, 47, 0.12);
  color: #b72f2f;
}
.log-danger strong {
  display: block;
