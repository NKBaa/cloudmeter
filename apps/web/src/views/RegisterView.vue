...
type RegistrationPolicy = { emailVerificationRequired: boolean; registrationEnabled: boolean }

const form = reactive({ displayName: '', email: '', password: '', code: '' })
...
<template><main class="auth-shell"><section class="auth-copy"><BrandMark/><div><p class="eyebrow">加入平台</p><h1>从一个应用开始你的工作区</h1><p class="lede">注册后即可查看管理员发布的应用模板，并管理自己的部署。</p></div><small>已有账户？<a href="/login">返回登录</a></small></section><section class="auth-form"><form :aria-busy="loading" @submit.prevent="register"><h2>创建账户</h2><p>{{ verificationRequired ? '使用真实邮箱完成验证后创建账户' : '使用真实邮箱接收平台通知' }}</p><label>姓名<input v-model="form.displayName" required maxlength="80" autocomplete="name" :disabled="formDisabled" /></label><label>邮箱<input v-model="form.email" type="email" required autocomplete="email" :disabled="formDisabled" /></label><div v-if="verificationRequired" class="code-field"><label>邮箱验证码<input v-model="form.code" required inputmode="numeric" maxlength="6" autocomplete="one-time-code" :disabled="formDisabled" /></label><button type="button" class="secondary send-btn" :disabled="formDisabled||sending||countdown>0" @click="sendCode">{{sending?'发送中':countdown>0?`${countdown}s`:'发送验证码'}}</button></div><label>密码<input v-model="form.password" type="password" required minlength="12" autocomplete="new-password" :disabled="formDisabled" /><small>至少 12 个字符</small></label><p v-if="message" class="message" role="status">{{message}}</p><button class="primary" :disabled="loading||formDisabled"><LoaderCircle v-if="loading" class="spin" :size="18"/><template v-else>{{submitLabel}}<ArrowRight v-if="registrationEnabled&&policyAvailable" :size="18"/></template></button></form></section></main></template>
