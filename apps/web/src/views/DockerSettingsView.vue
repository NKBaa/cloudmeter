...
const model = reactive<DockerSettings>({
  ...
  registryPasswordConfigured: false, httpProxy: '', httpsProxy: '', noProxy: 'localhost,127.0.0.1,::1',
  pullTimeoutSeconds: 300, detectedRegistryMirrors: [], detectedHttpProxy: '', detectedHttpsProxy: '',
  detectedNoProxy: '', lastCheckedAt: null, lastCheckError: '', daemonRestartRequired: false, daemonJson: '{}',
})
...
const detectedMirrorText = computed(() => model.detectedRegistryMirrors.length ? model.detectedRegistryMirrors.join('、') : '未探测到镜像加速源')
const restartActive = computed(() => restartRequest.value?.status === 'queued' || restartRequest.value?.status === 'running')
const restartStatus = computed(() => {
  if (restartConnectionLost.value && restartActive.value) return { label: '连接恢复中', detail: '平台服务正在切换，页面会自动重试连接。', tone: 'running' }
  if (!restartRequest.value) return null
  const status = restartRequest.value.status
  if (status === 'queued') return { label: '等待执行', detail: '重启任务已进入队列，Worker 即将处理。', tone: 'running' }
  if (status === 'running') return { label: '正在重启', detail: '平台控制面会短暂中断，请勿重复提交。', tone: 'running' }
  if (status === 'succeeded') return { label: '重启完成', detail: '平台服务已恢复，用户应用未受重启范围影响。', tone: 'success' }
  return { label: '重启失败', detail: restartRequest.value.lastError || '平台服务未能完成重启，请检查 Worker 日志。', tone: 'failed' }
})
...
function clearRestartTimer() { if (restartTimer) window.clearTimeout(restartTimer); restartTimer = undefined }
...
async function loadRestartStatus(polling = false) {
  try {
    ...
    if (restartActive.value) {
      ...
      clearRestartTimer()
      if (previousStatus && previousStatus !== result.request?.status && result.request?.status === 'succeeded') {
...
async function save() {
  try {
    busy.value = 'save'
    const registryMirrors = mirrorsText.value.split(/\r?\n|,/).map((item) => item.trim()).filter(Boolean)
    await api('/admin/settings/docker', {
      method: 'PUT',
      body: JSON.stringify({
        registryMirrors, defaultRegistry: model.defaultRegistry.trim(), registryUsername: model.registryUsername.trim(),
        registryPassword: model.registryPassword, httpProxy: model.httpProxy.trim(), httpsProxy: model.httpsProxy.trim(),
        noProxy: model.noProxy.trim(), pullTimeoutSeconds: model.pullTimeoutSeconds,
      }),
    })
    done('Docker 配置已保存；Registry 凭据和拉取策略立即生效，Daemon 配置仍需在宿主机应用')
    await load(true)
  } catch (value) { failed(value) } finally { busy.value = '' }
}
async function copyDaemonJSON() {
  try { await navigator.clipboard.writeText(model.daemonJson); done('daemon.json 示例已复制') }
  catch { failed(new Error('复制失败，请手动选择配置内容')) }
}
async function restartPlatform() {
  try {
    restartBusy.value = true
    const result = await api<{ request: RestartRequest }>('/admin/system/restart', { method: 'POST', body: '{}' })
    restartRequest.value = result.request
...
<template>
  <main class="workspace admin-workspace docker-settings-view">
    <header>
      ...
      <button class="danger-button compact platform-restart-button" :disabled="restartActive" @click="restartDialogOpen = true">
        <Power :class="{ 'power-pulse': restartActive }" :size="16" />{{ restartActive ? '重启进行中' : '系统重启' }}
      </button>
    </header>
    <p v-if="error" class="message sticky-message">{{ error }}</p>
    <p v-if="message" class="status-ok sticky-message">{{ message }}</p>

    <section v-if="restartStatus" :class="['platform-restart-status', restartStatus.tone]">
      ...
      <div><strong>{{ restartStatus.label }}</strong><p>{{ restartStatus.detail }}</p></div>
      <time v-if="restartRequest">{{ new Date(restartRequest.createdAt).toLocaleString('zh-CN') }}</time>
    </section>
    ...
    <div class="docker-settings-layout">
      <form class="form-panel docker-settings-form" @submit.prevent="save">
        <section class="docker-form-section">
          <div class="config-heading"><Network :size="18" /><div><strong>镜像来源</strong><small>一行一个 HTTPS 镜像加速地址；可维护私有 Registry 默认前缀</small></div></div>
          ...
          <label>默认镜像库 / 命名空间
            ...
            <small>产品版本填写相对镜像名时自动补全；完整镜像地址保持不变。</small>
          </label>
          <div class="field-row">
            <label>Registry 用户名<input v-model="model.registryUsername" autocomplete="username" /></label>
            <label>Registry 密码<input v-model="model.registryPassword" type="password" autocomplete="new-password" :placeholder="model.registryPasswordConfigured ? '已加密保存，留空保持不变' : '未配置'" /></label>
          </div>
        </section>

        <section class="docker-form-section">
          <div class="config-heading"><Container :size="18" /><div><strong>代理与拉取策略</strong><small>代理只供 Docker 拉取镜像，不注入用户应用容器</small></div></div>
          <div class="field-row">
        ...
        <div class="docker-form-actions">
          ...
          <button class="primary compact" :disabled="busy === 'save'"><Save :size="16" />{{ busy === 'save' ? '保存中' : '保存设置' }}</button>
        </div>
      </form>
      ...
      <aside class="docker-runtime-aside">
        <section class="form-panel daemon-status-card">
          <div class="daemon-status-heading">
            <span :class="['context-empty-icon', model.daemonRestartRequired && 'warning']"><CircleAlert v-if="model.daemonRestartRequired" :size="21" /><CheckCircle2 v-else :size="21" /></span>
        ...
        <section class="form-panel daemon-json-card">
          <div class="config-heading with-action"><Database :size="18" /><div><strong>daemon.json 示例</strong><small>将期望配置应用到宿主机</small></div><button class="icon-action" type="button" title="复制" @click="copyDaemonJSON"><Copy :size="16" /></button></div>
          <pre>{{ model.daemonJson }}</pre>
          <ol><li>写入宿主机 <code>/etc/docker/daemon.json</code></li><li>执行 <code>sudo systemctl restart docker</code></li><li>等待 Worker 下一次自动探测</li></ol>
        </section>
      </aside>
    </div>
    ...
    <section class="form-panel docker-images-panel">
      ...
      <div v-if="imageBusy==='load'&&!images.length" class="docker-image-list" aria-busy="true"><article v-for="index in 4" :key="index" class="skeleton-row"><div style="flex:1;display:grid;gap:8px"><span class="skeleton skeleton-title" style="width:40%"></span><span class="skeleton skeleton-text" style="width:26%"></span></div><span class="skeleton skeleton-text" style="width:80px"></span><span class="skeleton skeleton-text" style="width:110px"></span></article></div>
      ...
      <div v-else class="context-empty"><Container :size="23"/><p>没有可显示的 Docker 镜像，或 Worker 尚未完成首次同步。</p></div>
    ...
    <Transition name="dialog-fade">
      <div v-if="restartDialogOpen" class="dialog-backdrop" @click.self="restartDialogOpen = false">
        <section class="dialog-panel platform-restart-dialog" role="dialog" aria-modal="true" aria-labelledby="platform-restart-title">
          <header><div><p class="eyebrow">平台维护</p><h2 id="platform-restart-title">重启 CloudMeter 平台服务？</h2></div><button class="icon-action" title="关闭" @click="restartDialogOpen = false"><X :size="18" /></button></header>
          <div class="platform-restart-dialog-body">
            <p>Gateway、Web、API、应用路由、出口代理和 Worker 会依次重启，控制台可能短暂断开，页面会自动恢复连接。</p>
            <dl><div><dt>不会重启</dt><dd>PostgreSQL、Redis、Docker Engine、用户应用</dd></div><div><dt>不会应用</dt><dd>尚未写入宿主机的 daemon.json 配置</dd></div></dl>
          </div>
          <footer class="dialog-actions"><button class="secondary compact" :disabled="restartBusy" @click="restartDialogOpen = false">取消</button><button class="danger-button compact" :disabled="restartBusy" @click="restartPlatform"><Power :size="16" />{{ restartBusy ? '正在提交' : '确认重启' }}</button></footer>
        </section>
      </div>
    </Transition>
  </main>
