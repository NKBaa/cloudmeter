...
const ticketStats = computed(() => ({
  ...
  waiting: tickets.value.filter((item) => item.status === "waiting_user")
    .length,
  finished: tickets.value.filter(
...
async function loadDetail(id: string, quiet = false) {
  try {
    ...
    const data = await api<{ ticket: Ticket; messages: TicketMessage[] }>(
      prefix + id,
    );
    detail.value = data.ticket;
    messages.value = data.messages;
    if (!quiet || data.messages.length !== previousCount)
      await scrollConversation(quiet ? "auto" : "smooth");
    error.value = "";
  } catch (value) {
...
async function load(preserve = true, quiet = false) {
  try {
    ...
    const nextID =
      preserve && candidates.some((item) => item.id === selectedID.value)
        ? selectedID.value
        : candidates[0]?.id || "";
...
async function createTicket() {
  try {
    busy.value = "create";
    const created = await api<{ id: string; number: number }>("/tickets", {
      method: "POST",
...
async function reply() {
  if (!detail.value || !replyBody.value.trim() || busy.value === "reply")
    return;
  try {
    busy.value = "reply";
    const ticketID = detail.value.id;
    const prefix = props.admin ? "/admin/tickets/" : "/tickets/";
    await api(prefix + ticketID + "/messages", {
      method: "POST",
...
watch([filterStatus, searchQuery], async () => {
  ...
  if (!visibleTickets.value.some((item) => item.id === selectedID.value))
  ...
  <main class="workspace tickets-view">
    <header class="ticket-page-heading">
      <div>
        ...
        <p class="quiet">
          {{
            admin
              ? "集中处理用户问题，完整保留每一次沟通与状态变化。"
              : "问题、补充信息和平台回复都集中在同一条会话中。"
          }}
        </p>
      </div>
      <div class="ticket-header-actions">
        <button
          ...
          @click="load()"
        >
          <RefreshCw :class="{ spin: busy === 'list' }" :size="16" />刷新
        </button>
        <button
          v-if="!admin"
    ...
    <section class="ticket-summary-strip" aria-label="工单状态概览">
      <button
        ...
        @click="filterStatus = ''"
      >
        <span>全部工单</span><strong>{{ ticketStats.total }}</strong>
      </button>
      <button
        :class="{ active: filterStatus === 'open' }"
        @click="filterStatus = 'open'"
      >
        <span>{{ admin ? "待接入" : "待处理" }}</span
      ...
      >
        <span>处理中</span><strong>{{ ticketStats.active }}</strong>
      </button>
      <button
        :class="{ active: filterStatus === 'waiting_user' }"
        @click="filterStatus = 'waiting_user'"
      >
        <span>{{ admin ? "等待用户" : "等待我" }}</span
        ><strong>{{ ticketStats.waiting }}</strong>
      </button>
      <button
        :class="{ active: filterStatus === 'finished' }"
        @click="filterStatus = 'finished'"
      >
        <span>已完成</span><strong>{{ ticketStats.finished }}</strong>
      </button>
    </section>
    ...
    <Transition name="toast-pop">
      <p
        v-if="error || notice"
        :class="['ticket-toast', error ? 'error' : 'success']"
        role="status"
      >
        {{ error || notice }}
      </p>
    ...
    <section class="ticket-workbench">
      <section class="ticket-list-panel">
        <div class="ticket-list-heading">
          <div>
            <strong>{{ admin ? "服务队列" : "我的工单" }}</strong
            ><small>{{ visibleTickets.length }} / {{ tickets.length }}</small>
          </div>
        ...
        <div class="ticket-list">
          <button
            ...
            :key="item.id"
            :class="['ticket-list-item', selectedID === item.id && 'selected']"
            @click="loadDetail(item.id)"
          >
            <span class="ticket-list-top">
              <span
                ><b>#{{ item.number }}</b
                ><i v-if="needsAttention(item)"></i
                ><small>{{ responseHint(item) }}</small></span
              >
              <time>{{ formatTime(item.lastMessageAt) }}</time>
            </span>
            <strong class="ticket-list-subject">{{ item.subject }}</strong>
            <span v-if="admin" class="ticket-requester"
              ><span>{{ initials(item.requesterName) }}</span
              >{{ item.requesterName
              }}<small>{{ item.requesterEmail }}</small></span
            >
            <span class="ticket-last-message"
              ><b>{{ item.lastReplyStaff ? "平台" : "用户" }}</b
            ...
            <span class="ticket-list-meta">
              ...
              <span :class="['ticket-priority-dot', item.priority]">{{
          ...
          <div
            v-if="!visibleTickets.length && busy !== 'list'"
            class="ticket-list-empty"
          >
            <Inbox :size="22" />
            <strong>{{
              tickets.length
      ...
      <Transition name="panel-swap" mode="out-in">
        <article v-if="detail" :key="detail.id" class="ticket-detail-panel">
          <header class="ticket-detail-heading">
            <div class="ticket-detail-title">
              ...
              <div>
                ...
                <p v-if="admin">
                  <UserRound :size="14" />{{ detail.requesterName }} ·
                  {{ detail.requesterEmail }}
                </p>
              </div>
            </div>
            <div class="ticket-detail-badges">
              <span :class="['status-pill', statusClass(detail.status)]">{{
                statusLabel(detail.status)
              }}</span
              ><span class="ticket-priority"
                >{{ priorityLabel(detail.priority) }}优先级</span
              >
            </div>
          </header>
          <div class="ticket-detail-meta">
            ...
            <span
              ><Clock3 :size="14" />创建于
              {{ formatFullTime(detail.createdAt) }}</span
            >
            <span
          ...
          <div class="ticket-conversation" aria-live="polite">
            <div v-if="busy === 'detail'" class="ticket-conversation-loading">
              <RefreshCw class="spin" :size="18" />正在读取会话…
            </div>
            <div
            ...
            >
              ...
              <div class="ticket-message-content">
                <div>
                  ...
                  ><span>{{ messageRole(item) }}</span
                  ><time>{{ formatFullTime(item.createdAt) }}</time>
                </div>
                <p>{{ item.body }}</p>
              </div>
            </div>
          ...
          <footer class="ticket-composer">
            ...
            <form v-if="canReply" @submit.prevent="reply">
              <label
                ...
                ><textarea
                  ...
                  :placeholder="
                    admin
                      ? '说明处理进展、解决方法或需要用户补充的信息…'
                      : '补充现象、日志或复现步骤…'
                  "
                  @keydown.ctrl.enter.prevent="reply"
                  @keydown.meta.enter.prevent="reply"
                />
              </label>
              <div class="ticket-composer-actions">
                <small>{{ replyBody.length }} / 10000 · Ctrl + Enter 发送</small
                ><span
                  ><button
                    v-if="!admin"
                    type="button"
            ...
            <p v-else class="ticket-closed-note">
              <CheckCircle2
                :size="17"
              />工单已关闭。如问题仍未解决，请新建工单继续咨询。
            </p>
          </footer>
        </article>
        <div v-else class="ticket-detail-empty">
          <span class="context-empty-icon"
            ><MessageSquareText :size="24"
          /></span>
          <div>
            <h2>{{ tickets.length ? "选择一条工单查看会话" : "暂无会话" }}</h2>
            <p>
              {{
                admin
                  ? "工单进入队列后，可在这里查看上下文并回复用户。"
                  : "新建工单后，平台回复会集中显示在这里。"
              }}
            </p>
          </div>
    ...
    <Transition name="modal-pop">
      ...
      >
        <section
          class="secret-dialog ticket-create-dialog"
          role="dialog"
          aria-modal="true"
        ...
        >
          <header>
            <div>
              ...
              <h2 id="ticket-create-title">新建工单</h2>
            </div>
            <button
              class="icon-action"
              type="button"
          ...
          <form @submit.prevent="createTicket">
            ...
            <div class="field-row">
              <label
                >问题类型<select v-model="createForm.category">
                  <option
              ...
              ><label
                >优先级<select v-model="createForm.priority">
                  <option
            ...
            <label
              >问题描述<textarea
                v-model.trim="createForm.body"
                rows="7"
                maxlength="10000"
                required
                placeholder="请写明操作步骤、预期结果和实际现象；如有错误日志也请一并提供。"
              />
            </label>
            <p class="ticket-create-hint">
              <CircleAlert :size="15" />请勿在工单中提交密码、Token 或私钥。
            </p>
            <div class="deploy-dialog-actions">
              ...
              >
                取消</button
              ><button class="primary compact" :disabled="busy === 'create'">
                <Send :size="15" />{{
                  busy === "create" ? "提交中…" : "提交工单"
                }}
              </button>
            </div>
          </form>
