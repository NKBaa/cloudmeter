...
async function markPaid(order: Order) {
  try {
    busy.value = order.id;
    await api("/admin/payments/orders/" + order.id + "/mark-paid", {
      method: "POST",
    });
    order.status = "paid";
...
async function submitRefund() {
  ...
  try {
    busy.value = order.id;
    const result = await api<RefundResponse>(
      "/admin/payments/orders/" + order.id + "/refund",
      {
        method: "POST",
...
async function query(order: Order) {
  try {
    busy.value = order.id;
    const result = await api<{ providerStatus: string; message: string }>(
      "/admin/payments/orders/" + order.id + "/query",
      { method: "POST" },
    );
    message.value =
      "提供商状态：" + result.providerStatus + " · " + result.message;
    error.value = "";
  } catch (cause) {
    error.value = (cause as Error).message;
  } finally {
...
async function close(order: Order) {
  ...
  try {
    busy.value = order.id;
    await api("/admin/payments/orders/" + order.id + "/close", {
      method: "POST",
    });
    order.status = "closed";
    pendingOrderTotal.value = Math.max(0, pendingOrderTotal.value - 1);
...
function statusClass(status: string) {
  ...
  if (status === "pending" || status === "refunding" || status === "processing")
    return "pending";
...
function refundStatusLabel(status: string) {
  return (
    { processing: "处理中", succeeded: "退款成功", failed: "退款失败" }[
      status
    ] || status
  );
}
...
<template>
  <main class="workspace admin-workspace">
    <header>
      <div>
        ...
        <h1>充值与退款</h1>
        <p class="quiet">实时监控全站支付流水、充值订单与退款审计记录。</p>
      </div>
    ...
    <section class="payment-list">
      ...
      <article v-for="order in orders" :key="order.id" class="payment-row">
        <div class="payment-copy">
          ...
          <small
            >{{ order.provider === "manual" ? "人工充值" : "EPay" }} ·
            {{ date(order.createdAt) }}</small
          >
          <small v-if="order.refund" class="refund-reference"
            >退款 {{ order.refund.id.slice(0, 8) }} ·
            {{ refundStatusLabel(order.refund.status) }}</small
          >
        </div>
        ...
        <div class="payment-actions">
          ...
          <button
            v-if="order.status === 'paid' && order.provider === 'manual'"
            class="secondary compact"
            :disabled="busy === order.id"
      ...
      <article v-for="refund in refunds" :key="refund.id" class="refund-record">
        <div class="refund-record-summary">
          <div class="payment-copy">
            ...
            <small
              >订单 {{ refund.orderId.slice(0, 8) }} ·
              {{ refund.provider === "manual" ? "人工充值" : "EPay" }} ·
              {{ date(refund.createdAt) }}</small
            >
          </div>
          <b>¥ {{ money(refund.amountCents) }}</b>
          <span :class="['status-pill', statusClass(refund.status)]">{{
            refundStatusLabel(refund.status)
          }}</span>
          <button
            class="icon-action"
            :title="
              expandedRefunds.includes(refund.id)
                ? '收起退款详情'
                : '展开退款详情'
            "
            @click="toggleRefund(refund.id)"
          >
        ...
        <div
          v-if="expandedRefunds.includes(refund.id)"
          class="refund-record-detail"
        >
          <dl class="refund-metadata">
            ...
            <div>
              <dt>操作人</dt>
              <dd>{{ refund.requestedByEmail || "历史迁移" }}</dd>
            </div>
            <div>
              <dt>账本引用</dt>
              <dd>{{ refund.ledgerEntryId || "未写入" }}</dd>
            </div>
            <div>
              <dt>完成时间</dt>
          ...
          <ol class="refund-timeline">
            <li v-for="event in refund.events" :key="event.id">
              <Clock3 :size="15" />
              <span
                ><strong>{{ refundStatusLabel(event.toStatus) }}</strong
                ><small
                  >{{ event.message }} · {{ date(event.createdAt) }}</small
                ></span
              >
            </li>
          </ol>
  ...
  <Teleport to="body">
    <div v-if="refundTarget" class="dialog-backdrop" @click.self="cancelRefund">
      <section
        class="dialog-panel refund-dialog"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <form @submit.prevent="submitRefund">
          ...
          <p class="quiet">
            提交后会从账户钱包扣回本次充值金额，并写入不可变退款记录和事件时间线。
          </p>
          <div class="dialog-actions">
            ...
            <button
              class="primary"
              :disabled="busy === refundTarget.id || !refundReason.trim()"
            >
              <Undo2 :size="17" />确认退款
            </button>
          </div>
