...
function volumeGroups(appID: string): VolumeGroup[] {
  ...
  for (const volume of current.volumes) {
    groups.set(volume.name, {
      ...volume,
      active: true,
      backups: [],
      usageBytes: current.volumeUsageBytes?.[volume.name] || 0,
    });
  }
...
function volumeID(appID: string, volumeName: string) {
...
function latestBackup(group: VolumeGroup) {
  return group.backups[0];
}
...
function liveUsage(appID: string) {
  return Object.values(appData(appID).volumeUsageBytes || {}).reduce(
    (sum, value) => sum + Number(value || 0),
    0,
  );
}
function capacityBytes(appID: string) {
  return Math.max(0, appData(appID).capacityGiB || 0) * 1024 ** 3;
}
function totalUsage(appID: string) {
  return liveUsage(appID) + Number(appData(appID).backupUsageBytes || 0);
}
function remainingBytes(appID: string) {
  return Math.max(0, capacityBytes(appID) - totalUsage(appID));
}
...
function backupStatus(backup: Backup) {
  if (backup.deletionStatus === "queued" || backup.deletionStatus === "running")
    return "删除中";
...
function backupStatusClass(backup: Backup) {
  if (backup.deletionStatus === "queued" || backup.deletionStatus === "running")
    return "pending";
  if (backup.deletionStatus === "failed" || backup.status === "failed")
    return "danger";
  return backup.status === "succeeded" ? "active" : "pending";
}
const totalBackups = computed(() =>
  Object.values(data.value).reduce((sum, item) => sum + item.backups.length, 0),
);
...
async function load(silent = false) {
  try {
    ...
    const pairs = await Promise.all(
      result.apps.map(
        async (app) =>
          [app.id, await api<BackupData>(`/apps/${app.id}/backups`)] as const,
      ),
    );
    data.value = Object.fromEntries(pairs);
    if (
      expandedAppID.value &&
      !result.apps.some((app) => app.id === expandedAppID.value)
    )
      expandedAppID.value = "";
    error.value = "";
  } catch (value) {
...
async function createBackup(app: App, key: string) {
  try {
    ...
    error.value = "";
    await api(`/apps/${app.id}/backups`, {
      method: "POST",
      body: JSON.stringify({ volumeKey: key }),
    });
...
async function restore(app: App, backup: Backup) {
  if (
    !confirm(
      `确定用 ${formatDate(backup.createdAt)} 的备份恢复 ${app.slug} / ${backup.volumeKey}？恢复期间应用会短暂离线。`,
    )
  )
    return;
  try {
    ...
    error.value = "";
    await api(`/apps/${app.id}/backups/${backup.id}/restore`, {
      method: "POST",
...
async function removeBackup(app: App, backup: Backup) {
  ...
  if (
    !confirm(
      `${retry ? "重新尝试删除" : "确定删除"} ${app.slug} / ${backup.volumeKey} 在 ${formatDate(backup.createdAt)} 创建的备份？归档清理后不能恢复。`,
    )
  )
    return;
  try {
  ...
  <main class="workspace backups-view">
    ...
    <header class="page-heading">
      <div>
        ...
        <h1>备份与恢复</h1>
        <p class="quiet">
          应用、数据卷和备份记录逐层展开；备份与应用数据共享同一容量池，不重复收取存储费。
        </p>
      </div>
    ...
    <section
      ...
      aria-busy="true"
    >
      <article v-for="index in 3" :key="index" class="skeleton-row">
        <span
          class="skeleton"
          style="width: 38px; height: 38px; border-radius: 11px"
        ></span>
        <div style="flex: 1; display: grid; gap: 8px">
          <span class="skeleton skeleton-title" style="width: 34%"></span
    ...
    <section v-if="apps.length" class="backup-application-list">
      <div class="backup-page-summary">
        <span>{{ apps.length }} 个应用</span
        ><span>{{ totalBackups }} 条可见备份</span
        ><small>卷数据 + 成功备份共同占用用户选择的共享数据卷容量</small>
      </div>
      <article
        v-for="app in apps"
        :key="app.id"
        :class="[
          'backup-application-item',
          { expanded: expandedAppID === app.id },
        ]"
      >
        <button
          class="backup-application-row"
          type="button"
          :aria-expanded="expandedAppID === app.id"
          @click="toggleApp(app.id)"
        >
          <span class="backup-app-icon"><Database :size="19" /></span>
          <span class="backup-app-identity"
            ><strong>{{ app.slug }}</strong
            ><small>{{ app.productSlug }}</small></span
          >
          <span class="backup-app-capacity"
            ><small>共享容量</small
            ><strong>{{ appData(app.id).capacityGiB || 0 }} GiB</strong
            ><i><em :style="{ width: usagePercent(app.id) + '%' }"></em></i
          ></span>
          <span class="backup-app-facts"
            ><span
              ><strong>{{ volumeGroups(app.id).length }}</strong
              ><small>数据卷</small></span
            ><span
              ><strong>{{ appData(app.id).backups.length }}</strong
              ><small>备份</small></span
            ></span
          >
          <span
            :class="[
              'status-pill',
              app.status === 'running' ? 'active' : 'suspended',
            ]"
        ...
        <Transition name="backup-detail">
          <div
            ...
            class="backup-application-detail"
          >
            <div class="backup-capacity-strip">
              <span
                ><small>应用数据</small
                ><strong>{{ formatBytes(liveUsage(app.id)) }}</strong></span
              >
              <span
                ><small>备份归档</small
                ><strong>{{
                  formatBytes(appData(app.id).backupUsageBytes)
                }}</strong></span
              >
              <span
                ><small>合计占用</small
                ><strong>{{ formatBytes(totalUsage(app.id)) }}</strong></span
              >
              <span
                ><small>剩余容量</small
                ><strong>{{
                  formatBytes(remainingBytes(app.id))
                }}</strong></span
              >
            </div>
            <section class="backup-subsection">
              <div class="backup-subsection-heading">
                <div>
                  <strong>按数据卷管理</strong
                  ><small>点击数据卷展开按时间倒序排列的备份</small>
                </div>
                <span>{{ volumeGroups(app.id).length }} 个</span>
              </div>
              <div
                v-if="volumeGroups(app.id).length"
                class="backup-volume-groups"
              >
                <article
                  v-for="group in volumeGroups(app.id)"
                  :key="group.name"
                  :class="[
                    'backup-volume-group',
                    {
                      expanded:
                        expandedVolumeID === volumeID(app.id, group.name),
                    },
                  ]"
                >
                  <div class="backup-volume-row">
                    <button
                      class="backup-volume-toggle"
                      type="button"
                      :aria-expanded="
                        expandedVolumeID === volumeID(app.id, group.name)
                      "
                      @click="toggleVolume(app.id, group.name)"
                    >
                      <span class="backup-row-primary"
                        ><HardDrive :size="16" /><span
                          ><strong>{{ group.name }}</strong
                          ><small>{{ group.mountPath }}</small></span
                        ></span
                      >
                      <span
                        ><small>卷占用</small
                        ><strong>{{
                          formatBytes(group.usageBytes)
                        }}</strong></span
                      >
                      <span
                        ><small>备份记录</small
                        ><strong>{{ group.backups.length }} 条</strong></span
                      >
                      <span
                        ><small>最近备份</small
                        ><strong>{{
                          formatDate(latestBackup(group)?.createdAt)
                        }}</strong></span
                      >
                  ...
                  <Transition name="backup-detail">
                    <div
                      v-if="expandedVolumeID === volumeID(app.id, group.name)"
                      class="backup-volume-history"
                    >
                      <div
                        v-if="group.backups.length"
                        class="backup-history-table"
                      >
                        <div class="backup-table-head">
                          <span>备份时间</span><span>归档容量</span
                        ...
                        >
                          <span class="backup-row-time"
                            ><strong>{{ formatDate(backup.createdAt) }}</strong
                            ><small v-if="backup.lastError">{{
                              backup.lastError
                            }}</small></span
                          >
                          <span>{{ formatBytes(backup.sizeBytes) }}</span>
                          <span
                            :class="['status-pill', backupStatusClass(backup)]"
                            ...
                            <button
                              ...
                              :disabled="
                                ...
                                ['queued', 'running'].includes(
                                  backup.deletionStatus || '',
                                ) ||
                                busy !== ''
                              "
                              @click="removeBackup(app, backup)"
                            >
                              <Trash2 :size="16" />
                            </button>
                          </span>
                      ...
                      <div v-else class="backup-history-empty">
                        这个数据卷还没有备份。点击“立即备份”创建第一条可恢复记录。
                      </div>
                    </div>
                  </Transition>
                </article>
              ...
              <div
                v-else
                class="context-empty compact-empty backup-inline-empty"
              >
                <Database :size="21" />
                <div>
                  <strong>此应用没有可备份的数据卷</strong>
                  <p>只有管理员在产品版本中声明的持久卷才会出现在这里。</p>
                </div>
              </div>
            </section>
          </div>
    ...
    <section v-else-if="!loading" class="nextdev-card p-0">
      <div class="card-header-bar flex items-center justify-between">
        <div class="card-title-group">
          <span class="eyebrow">BACKUPS · 数据备份与恢复</span>
          <h3>数据卷备份空间</h3>
        </div>
      ...
      <div class="card-inner-body">
        <div class="empty-state-nextdev">
          ...
          <h4>暂无备份空间</h4>
          <p>先部署一个包含持久卷的应用，实例运行后即可在此按需创建快照或执行一键恢复。</p>
