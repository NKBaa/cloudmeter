...
<template>
  <!-- Public navigation keeps documentation available before sign-in. -->
  <main class="public-home">
    <nav v-if="!inShell" class="home-nav"><BrandMark /><div><a href="#features">产品能力</a><RouterLink to="/docs">文档</RouterLink><RouterLink to="/console">控制台</RouterLink></div><div class="home-nav-actions"><button class="icon-action theme-toggle" :title="theme === 'dark' ? '切换浅色主题' : '切换深色主题'" @click="toggleTheme"><Sun v-if="theme === 'dark'" :size="16" /><Moon v-else :size="16" /></button><RouterLink class="ghost-link" to="/login"><LogIn :size="16" />登录</RouterLink><RouterLink class="solid-link" to="/register">开始使用<ArrowRight :size="16" /></RouterLink></div></nav>
