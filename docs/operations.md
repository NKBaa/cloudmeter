...
### 组合部署（多容器）
产品版本的 `runtimeSpec.companions` 可声明最多 8 个伴随容器。每项使用固定的 `key`、`serviceName`、带版本的 `image`、内网 `ports`、资源、环境变量、命令和共享卷挂载；挂载只能引用主容器已声明的卷键，禁止宿主机路径、host network 和 Docker Socket。

组合版本仍然只创建一个用户应用和一个 Release。Worker 在同一用户网络中先拉取并启动伴随容器，再启动主容器；主容器是唯一公网路由入口，伴随容器不会获得宿主机端口。容器可通过稳定 DNS 名称访问，例如 AstrBot + NapCat 预设中的 `http://napcat:6099`。健康检查要求主容器和全部伴随容器均就绪，否则整个 Release 回滚并清理。停止、删除、更新、恢复、订阅暂停、失败回滚和日志采集均按容器组执行。

管理端提供“AstrBot + NapCat”预设；创建版本前请确认镜像版本标签与目标架构，并按实际版本调整端口和健康检查。
...
后台“备份与恢复”按“应用 → 数据卷 → 备份记录”三级展示。数据卷名称是备份归类的稳定键，卷内记录按创建时间倒序排列；单条删除只会创建异步物理清理任务，数据库备份历史和审计记录保留不变。清理失败会进入“删除失败”，再次点击删除即可重试。

每个当前数据卷支持导入 `tar.gz` 归档，单文件最大 2 GiB；服务端会校验 gzip/tar 结构、路径穿越、链接或设备文件、条目数量和解压后大小。导入成功后生成正常的不可变备份记录并计入共享数据卷容量，用户仍需通过原有恢复操作明确覆盖运行卷。每条未删除的成功备份可导出原始 `tar.gz` 归档。API 仅挂载备份存储卷，不持有 Docker Socket；Compose 的 `backup-storage-init` 负责初始化归档目录权限。非正式 Compose 项目若使用作用域备份卷，应把 `BACKUP_STORAGE_MOUNT_VOLUME` 设置为 Worker 实际使用的作用域卷名。

应用部署时选择的共享数据卷容量是该应用的数据容量上限；当前运行卷实际占用与保留的成功备份归档共同计入这个容量池，存储容量只计费一次。备份操作本身仍可按 `backup.operation` 单独计费，不再产生新的 `backup.storage.gib_days` 用量；历史账单中的旧费用项不会被改写。控制台会显示容量、应用数据占用、备份占用和剩余容量。

恢复期间应用进入 `updating`，Worker 停止容器、覆盖目标卷、重新启动并等待健康检查；成功后恢复为 `running`。删除中的备份不可恢复，删除失败或排队中的归档不会被计入可恢复记录。备份 helper 使用固定 Digest 镜像和无网络模式，用户容器不会获得 Docker Socket。

“我的应用”中的“编辑配置并重新部署”会读取当前成功发布配置并预填。管理员在产品版本中标记为可编辑的 CPU、内存、共享数据卷、启动命令、环境变量和依赖可以调整；镜像、内网端口、挂载路径、健康检查等固定字段仍由管理员模板控制。共享数据卷只能扩容，不能缩容。保存后会创建新的不可变 Release 并进入正常部署状态机。

应用 Secret 用于 API Key、Token、数据库密码等敏感值。Secret 以加密版本保存，接口和页面不返回明文；修改会追加新版本，运行中的容器继续使用当前 Release 引用的旧版本，只有重新部署后才会注入最新版本。
...
迁移 58 为新旧安装预置了标准费用项。当前只对持久数据卷生成 `storage.data.gib_days`，镜像和容器可写层由平台承担，不生成 `storage.system.gib_days`。旧 `storage.system.gib_days`、`storage.gib_days` 与 `backup.storage.gib_days` 仅用于展示历史账单，不再产生新用量；应用数据与保留备份共用用户选择的数据卷容量池，容量只计费一次，备份操作仍按 `backup.operation` 计费。
